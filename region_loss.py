...
def build_targets(pred_boxes, target, conf, anchors, num_anchors, feature_size, input_size, ignore_thresh):
    ...
    # gx = (tboxes[:, 1] * feature_size[1]).int()
    # gy = (tboxes[:, 2] * feature_size[0]).int()
    # gw = (tboxes[:, 3] * input_size[1]).int()
    # gh = (tboxes[:, 4] * input_size[0]).int()
    aw = anchors[:, 0]
    ah = anchors[:, 1]
    nbox = tboxes.size(0)
    ...
    for i, iou in enumerate(ious):
        if (iou > ignore_thresh).sum():
            noobj_mask[ind_B[i:i + 1], (iou > ignore_thresh).nonzero(as_tuple=False).squeeze(1), gj[i:i + 1], gi[i:i + 1]] = 0
