...
def train(epoch):
    ...
    flops, params = profile(model, input_size=(data, metax, mask))
    print('flops:', flops, 'params:', params)
...
#
# x = Variable(torch.randn(8, 3, 256, 256))
# metax = Variable(torch.randn(8, 3, 256, 256))
# mask = Variable(torch.randn(8, 1, 256, 256))
# x = x.cuda()
# metax = metax.cuda()
# mask = mask.cuda()
# y = net(x, metax, mask)
